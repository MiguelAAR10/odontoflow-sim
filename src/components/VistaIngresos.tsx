...
/**
 * Vista de ingresos: qué plata está de verdad en riesgo y qué la pone en riesgo.
 *
 * Abre con "En riesgo real": la suma de soles × riesgo de las citas vivas. Es
 * menor que el agendado (que es el total) y por eso informa: dice cuánto puede
 * caerse de verdad, no cuánto está en juego en abstracto. El resto de la pantalla
 * existe para explicar ese número.
 */

/** Estados sobre los que recepción puede hacer algo ahora mismo. */
    ...
    <>
      ...
      <div className="flex flex-wrap items-end gap-6 px-0.5 pb-4">
        <div>
          ...
          <div className="rotulo mt-1.5 text-[9.5px] text-ink-3">En riesgo real</div>
        </div>
        <p className="w-full pb-1 text-[12.5px] leading-relaxed text-ink-2 sm:w-auto sm:max-w-[40ch]">
          De <b className="font-semibold text-ink">{soles(totales.agendado)}</b> agendado,{" "}
          <b className="font-semibold text-ok">{soles(totales.confirmado)}</b> ya está confirmado y{" "}
          <b className="font-semibold text-late">{soles(totales.vencido)}</b> venció el plazo de
          respuesta. Lo que está en juego de verdad es lo que todavía puede caerse.
        </p>
      </div>
        ...
        <section className="overflow-hidden rounded-[9px] border border-line bg-panel">
          <header className="flex items-center gap-2 border-b border-line bg-panel-2 px-3 py-2.5">
            ...
            <span className="tabular ml-auto text-[11px] text-ink-3">
              {riesgosas.length > 0 ? `${riesgosas.length} requieren seguimiento` : "al día"}
            </span>
          </header>
          ...
          {riesgosas.length === 0 ? (
            <p className="px-3 py-7 text-[12.5px] leading-relaxed text-ink-3">
              Nada en riesgo ahora mismo. Mueve la línea de tiempo para ver al sistema trabajar.
            </p>
          ) : (
            riesgosas.map((c) => (
              <button
                ...
                className="grid w-full grid-cols-[46px_1fr_74px] items-center gap-2.5 border-b border-line px-3 py-2 text-left transition hover:bg-ok-soft last:border-b-0 sm:grid-cols-[54px_1fr_78px_78px_58px]"
              >
                ...
                <span className="tabular text-right text-[12.5px] font-semibold">{soles(c.soles)}</span>
        ...
        <div className="flex flex-col gap-3.5">
          ...
          <section className="overflow-hidden rounded-[9px] border border-line bg-panel">
            ...
            <div className="flex items-center gap-3.5 p-3">
              <div>
                <div className="display text-[30px] leading-none tracking-[-0.025em] text-ok">
                  {soles(rescatado)}
                </div>
            ...
            <p className="border-t border-line px-3 py-1.5 text-[10px] leading-relaxed text-ink-4">
