...
export function Timeline({
  ...
  return (
    <div className="flex shrink-0 items-center gap-4 bg-dark px-4 py-2.5">
      ...
      >
        {/* raíl */}
        <div className="absolute inset-x-0 top-3 h-1 rounded-full bg-dark-3" />
        <div
          className="absolute top-3 left-0 h-1 rounded-full bg-ok-line transition-[width] duration-300"
        ...
        {dias.map((d) => (
          <span
            key={d.getTime()}
            className="absolute top-1.5 h-2 w-px bg-dark-3"
        ...
        {citas.map((c) => (
          <span
            key={c.id}
            className="absolute top-0.5 h-[7px] w-[3px] rounded-[1px] transition-colors duration-300"
            style={{
              left: `${pct(c.startsAt)}%`,
        ...
        {/* pomo */}
        <span
          className={`absolute top-1.5 -ml-2 h-4 w-4 rounded-full border-[3px] border-ok bg-white shadow-md transition-transform ${
      ...
      <span className="tabular w-24 text-right text-[12.5px] whitespace-nowrap text-[#e8efec]">
