 ...
 */
export function useContador(valor: number, ms = 460) {
...
export function useSubida(valor: number) {
  ...
  useEffect(() => {
    if (valor > anterior.current) {
      setSubio(true);
      const t = setTimeout(() => setSubio(false), 540);
      anterior.current = valor;
...
/** Dispara un pulso cuando `clave` cambia. Para el feedback del reloj al avanzar. */
export function usePulso(clave: number) {
  const [activo, setActivo] = useState(false);
  const anterior = useRef(clave);
  useEffect(() => {
    if (clave !== anterior.current) {
      anterior.current = clave;
      setActivo(true);
      const t = setTimeout(() => setActivo(false), 620);
      return () => clearTimeout(t);
    }
  }, [clave]);
  return activo;
